...
│  BiomeRules        TargetResolver      PlantSpawner     │
│  PlantRegistry     BiomeTransition     WorldGenScanner  │
│                                                        │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
│  │  树木生长     │  │  世界生成     │  │  客户端视觉   │  │
│  │  plant/tree/ │  │  worldgen/   │  │  client/     │  │
│  │  SpaceColon  │  │  BiomeMod    │  │  Visual      │  │
│  └──────────────┘  └──────────────┘  └──────────────┘  │
│                                                        │
│  ┌──────────────────────────────────────────────────┐   │
│  │  Mixin 层: SaplingBlock / BlockRender / worldgen │   │
│  └──────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────┘
```
...
| [plant-lifecycle-system.md](plant-lifecycle-system.md) | 植物适配器模式、生命周期追踪、WorldGen 扫描 | 理解植物如何被识别和追踪 |
| [tree-growth-system.md](tree-growth-system.md) | 空间定殖树生长、渐进生长管线、调试命令 | 理解树如何生长、添加新树种 |
...
**已完成:**
- Mod 引导、chunk 数据附件、JSON 配置加载（20 个演替路径）
...
- 负向退化 → fallback 群系
- 树生命周期 Phase 1-4：Mixin 拦截 + 渐进生长 + 空间定殖算法 + 死亡/腐烂
- 空间定殖树算法：9 种树种（含 1x1 和 2x2），DT 风格离散方向原木生长 + 末端叶片簇 + 包络体密度衰减
...
- BiomeModifier 树替换系统（CancelVanillaTrees + AddEcofluxTrees）
- EcofluxTreeFeature：世界生成阶段自定义 SC 树放置
- WorldGenVegetationScanner：chunk 加载时扫描原版生成植被并注册追踪
- 区块生成优化（PENDING_TREES 桥接 decoration→chunk-load）
- BiomeRules 群系植物规则系统（64 个群系配置文件）
- 植物采样器 `/ecoflux sample`：校准 BiomeRules 配置
- 性能分析器 `/ecoflux profile`
- 钠 (Sodium) 模组兼容 mixin
