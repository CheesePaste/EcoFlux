...
```
api/           公共 API（事件 / adapter 接口 / TreeGrowthProfileRegistry / 数据 record）
...
7. **Capability > type ID**：核心管线检查 `VegetationAdapterCapability`（IS_SAPLING/HAS_STRUCTURE/...）而非硬编码 adapter type ID，外部 mod 适配器声明 capability 即可参与。
8. **事件驱动扩展**：NeoForge 事件（VegetationLifecycleEvent / SuccessionEvent / TreeGrowthEvent）在关键生命周期节点发射，外部 mod 通过 `@SubscribeEvent` 订阅，无需 mixin。
